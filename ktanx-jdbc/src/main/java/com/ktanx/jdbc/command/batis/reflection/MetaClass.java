...
public class MetaClass {
    ...
    public Method getReadMethod(String field) {
        if (!fields.contains(field) || readMethods.get(field) == null) {
            throw new KtanxJdbcException(String.format("属性[%s]在类[%s]中不存在或没有getter方法", field, typeClass.getName()));
        }
    ...
    public Method getWriteMethod(String field) {
        if (!field.contains(field) || writeMethods.get(field) == null) {
            throw new KtanxJdbcException(String.format("属性[%s]在类[%s]中不存在或没有setter方法", field, typeClass.getName()));
        }

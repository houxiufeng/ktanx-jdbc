...
public abstract class AbstractCommandContextBuilder implements CommandContextBuilder {
    ...
    public CommandContext build(CommandTable commandTable) {
        ...
        if (isCommandUpperCase) {
            commandContext.setCommand(StringUtils.upperCase(commandContext.getCommand()));
        } else {
            commandContext.setCommand(StringUtils.lowerCase(commandContext.getCommand()));
        }
    ...
    protected String getPkField(CommandTable commandTable) {
        return MappingCache.getPkField(commandTable);
    }
    ...
    protected String getPkColumn(CommandTable commandTable) {
        return MappingCache.getPkColumn(commandTable);
    }
    ...
    protected String getColumn(CommandTable commandTable, String field) {
        return MappingCache.getColumn(commandTable, field);
    }
    ...
    protected String getTableName(CommandTable commandTable) {
        if (StringUtils.isBlank(commandTable.getTableName())) {
            String tableName = MappingCache.getTableName(commandTable);
    ...
    protected Set<String> getClassFields(CommandTable commandTable) {
        return MappingCache.getClassFields(commandTable);
    }
    ...
    protected CommandContext getGenericCommandContext(CommandTable commandTable) {
        ...
        commandContext.setType(commandTable.getType());
        ...
        if (this.keyGenerator == null || this.keyGenerator.isPkValueByDb()) {
    ...
    protected Object[] decideNativeField(CommandTable commandTable, String name, Object value) {
        ...
        GenericTokenParser tokenParser = new GenericTokenParser(NATIVE_FIELD_OPEN_TOKEN, NATIVE_FIELD_CLOSE_TOKEN, new FieldTokenHandler(commandTable));
